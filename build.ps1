...
properties {
    ...
    # this should line up fairly well with the version of DNM
    # that the contrib build depends on
	$public_version = "0.82"
...
task RunTests {
    ...
    foreach ($testDll in (Get-ChildItem -Path $build_dir\*.UnitTests*.dll)) {
        if ($nunitLauncher -ne $null) {
            # run tests using the TeamCity NUnit runner
            exec { & $nunitLauncher v2.0 x86 NUnit-2.4.8 $testDll }
        } else {
            # run tests using our own copy of NUnit
            exec { & "$source_dir\tools\nunit\nunit-console.exe" $testDll /labels /xml=$artifact_dir\TestResults.xml } `
...
task Compile -depends Init {
	...
	# build the solution
	exec { msbuild ""$source_dir\DotNetMigrations.Contrib.sln"" /m /nologo /t:Rebuild /p:Configuration=$configuration /p:OutDir=""$build_dir\\"" }
...
task Init -depends Clean {
	...
	Generate-Assembly-Info `
        -File "$source_dir\DotNetMigrations.Contrib\SharedAssemblyInfo.cs" `
        -Title "DotNetMigrations Contrib $info_version" `
        -Description "" `
        -Company "" `
